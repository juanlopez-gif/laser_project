...
def verify_overlap_quality(H_left, H_right, overlap_px, row_shift, delta_z, 
    ...
    if output_dir is not None:
        ...
        save_overlap_error_csv(
            ...
            valid_mask, pixel_size_mm, output_path / f"overlap_{pair_name}_errors.csv"
        )
        ...
        plot_overlap_diagnostics(
            ...
            output_path / f"overlap_{pair_name}_diagnostics.png"
        )
        ...
        for i in range(rows):
            for j in range(cols):
                if valid_mask[i, j]:
                    ...
                    error_um = diff_mm[i, j] * 1000 if not np.isnan(diff_mm[i, j]) else 0.0
                    f.write(f"{x_mm:.6f},{y_mm:.6f},{z_left:.6f},{z_right:.6f},{error_um:.3f}\n")
    
    logger.info(f"Saved error CSV: {Path(filepath).name}")


def plot_overlap_diagnostics(left_overlap, right_overlap, diff_um, 
    ...
    valid_mask = ~np.isnan(left_overlap) & ~np.isnan(right_overlap)
    diff_2d[valid_mask] = (left_overlap[valid_mask] - right_overlap[valid_mask]) * 1000  # µm
    ...
    for row_idx in rows_to_plot:
        if row_idx < rows:
            ...
            axes[1, 0].plot(x_mm, right_overlap[row_idx, :]*1000, 
