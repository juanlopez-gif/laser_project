...
def assemble_stitched_heightmap(H_left, H_right, overlap_px, row_shift, delta_z):
    """
    ✅ VERSIÓN COMPLETAMENTE CORREGIDA V2
    ...
    # ✅ Extraer las porciones alineadas de ambos heightmaps
    left_data = H_left[left_start_row:left_start_row+final_rows, :]
    right_data = H_right_corrected[right_start_row:right_start_row+final_rows, :]
    
    # ✅ PASO 1: Copiar parte NO-overlap de H_left
    H_stitched[:, :H_left.shape[1]-overlap_px] = left_data[:, :-overlap_px]
    
    # ✅ PASO 2: Promediar región de overlap
    left_overlap = left_data[:, -overlap_px:]
    right_overlap = right_data[:, :overlap_px]
    ...
    # Escribir overlap promediado
    start_col = H_left.shape[1] - overlap_px
    H_stitched[:, start_col:start_col+overlap_px] = overlap_avg
    
    # ✅ PASO 3: Copiar parte NO-overlap de H_right
